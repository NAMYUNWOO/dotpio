...
- GeneratedAt(UTC): 2026-03-25T11:35:35.572385Z
- Status: **OK**
- Window: last 7 days (max 120 commits)
- Checked commits: 120
- Portal prompt commits: 45
- Dominant mode commits: compact=19, detailed=1, neutral=25
- MODE TREND: **COMPACT**
- PRESSURE BAND: **HIGH** (edits +19 / -2 / net 17)
...
- ARW AUTO PLAN CONF MOMENTUM: **FREEZE** (lock-or-candidate-suppressed; oscillating=False streak=20 confΔ=+0 planΔ=+0 driftRisk=HIGH offlineOnly=True)
- ARW MOMENTUM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_ARW_MOMENTUM_ALIAS enabled=False full=FREEZE)
- ARW MOMENTUM SCORE: **93** (base=85 confΔ=+0 planΔ=+0 streak=20 driftRisk=HIGH parity=LOCK)
...
- AMBIENT RAMP WHY REC CONF STREAK: **20** (suppress=FALSE threshold=3 reason=confidence-streak-extended)
...
- PULSE REMAP MOMENTUM SUPPRESS: **ARM** (freeze-streak-building; streak=1 threshold=2 suppress=FALSE offlineOnly=True)
- PRM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_PULSE_REMAP_MOMENTUM_ALIAS enabled=False)
- FOCUS: **PORTAL** (portal=241 | alt=22 | pressure=17)
- FOCUS STREAK: **2**
- FOCUS SHIFT: **PRESSURE->PORTAL**
- FOCUS VOL: **STEADY** (switches=3/36 ratio=0.083)
- ROUTE ACTION: **PORTAL_AUDIT** (portal-family tokens dominate top movers)
- ACTION CONF: **LOW** (dom=0.861 spread=219 driftSpread=45)
- FOCUS BAL: **86%** (top=241 total=280 dom=0.861)
- FOCUS ENTROPY: **MID** (norm=0.454 raw=0.72 max=1.585)
...
- DRIFT MOMENTUM: **RISING** (recent=2.913 older=0.727 delta=2.186)
- ACTION STABILITY: **WATCH** (retune-watch-needed; conf=LOW vol=STEADY momentum=RISING)
- PRESSURE LAG: **FAST** (churn=17 momentum=RISING |Δ|=2.186)
...
- ROUTE PULSE LINK MODE STREAK: **60** (mode-stable-extended; current=IDLE prior=IDLE priorStreak=59 loaded=True)
...
- ROUTE PULSE LINK MODE FIT: **SYNC** (mode-stable-multi-window; mode=IDLE drift=+0 streak=60)
...
- WHAT-IF FALLBACK ALT2: **NONE** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_FALLBACK_ALT2 enabled=False fallback=NONE scores=portal:241 alt:22 pressure:17)
...
- ARW AUTO PLAN CANDIDATE SUPPRESS: **FALSE** (streak=20 threshold=3)
...
- LBA: **OFF** (flag=DOTPIO_EXPERIMENT_LANE_BUCKET_AGE_ALIAS enabled=False sys=999h dw=16h cv=25h)
- LANE BUCKET AGE: **SYSTEMS/OPS 999H | DESIGN/WORLD 16H | COMBAT/VFX 25H** (status=GAP window=24h)
...
- LANE PRIORITY REC CONF: **HIGH** (worstAge=999h momentumGap=3h reason=lane priority signal is strongly separated by age/momentum)
...
- LPR HYS THRESH REC: **LPR HYS THRESH REC:RAISE** (base=12 rec=16 floor=14 ceil=24 priorWindow=True volSpan=3 maxAbsMom=3 ageSpread=983 learn=high volatility widened window using regime step-size memory reason=lane-age volatility is high; raise threshold to suppress flapping)
...
- PULSE HEAT FX COMPACT-BUDGET DRIFT: **SPIKE** (pulse-heat-fx churn is likely competing with compact prompt budget; compactNet=+261 familyNet=+7 churn=7)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: **SPIKE** (route-glow-fx churn is likely competing with compact prompt budget; compactNet=+261 familyNet=+15 churn=15)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: **SPIKE** (rail-mode churn is likely competing with compact prompt budget; compactNet=+261 familyNet=+5 churn=9)
- STICKY TOKENS: **12**
- ANOMALY: **ON** (sticky=12/3 pressure=17/5)
- ANOMALY CONF: **HIGH** (triggers=2 gap=21)
- ROUTE VIBE DRIFT: **CALM +0 | EDGE +0 | DOOM +0**

## Token Totals (added/removed/net)
- Compact: +273 / -12 / net 261
- Detailed: +208 / -9 / net 199
- Shared: +13 / -3 / net 10

## Top Token Movers (net ±)
- `P:` net +17 (added 19, removed 2)
...
- LPR HYS WINDOW: LPR HYS WINDOW:BASE (floor=14, ceil=24, span=10, reason=adaptive window remains near baseline spread)
- LPR HYS WINDOW Δ: +0 (current=BASE, prior=BASE, loaded=True)
- PULSE HEAT FX COMPACT-BUDGET DRIFT: SPIKE (compactNet=+261, familyNet=+7, churn=7)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: SPIKE (compactNet=+261, familyNet=+15, churn=15)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: SPIKE (compactNet=+261, familyNet=+5, churn=9)
