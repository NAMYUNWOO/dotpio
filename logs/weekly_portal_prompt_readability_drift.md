...
- GeneratedAt(UTC): 2026-03-26T09:34:56.727464Z
- Status: **OK**
- Window: last 7 days (max 120 commits)
- Checked commits: 120
- Portal prompt commits: 16
- Dominant mode commits: compact=5, detailed=1, neutral=10
- MODE TREND: **COMPACT**
- PRESSURE BAND: **HIGH** (edits +17 / -2 / net 15)
- DRIFT RISK: **HIGH** (score=39 | imbalance=24 | pressure=15)
...
- ARW AUTO PLAN CONF MOMENTUM: **FREEZE** (lock-or-candidate-suppressed; oscillating=False streak=33 confΔ=+0 planΔ=+0 driftRisk=HIGH offlineOnly=True)
- ARW MOMENTUM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_ARW_MOMENTUM_ALIAS enabled=False full=FREEZE)
- ARW MOMENTUM SCORE: **93** (base=85 confΔ=+0 planΔ=+0 streak=33 driftRisk=HIGH parity=LOCK)
...
- AMBIENT RAMP WHY REC CONF STREAK: **33** (suppress=FALSE threshold=3 reason=confidence-streak-extended)
- URGENCY STACK PRUNING REC: **PARITY>FX>DETAIL** (protect-core-detailed-under-high-drift; parityChurn=9 fxChurn=4 detailedChurn=13 offlineOnly=True)
...
- DMG COMBO CONF COACH FALLBACK: **LOCK YOUR BREATH. BANK SAFE HITS. SAME CALL x9.** (streak-rising-lock-in-reminder; streak=9 streakDelta=+1 regime=CALM killHeatVol=0 offlineOnly=True)
...
- PULSE REMAP MOMENTUM SUPPRESS: **SUPPRESS** (freeze-streak-threshold-met; streak=14 threshold=2 suppress=TRUE offlineOnly=True)
- PRMS: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_PULSE_REMAP_MOMENTUM_SUPPRESSION_ALIAS enabled=False full=SUPPRESS)
- PULSE REMAP SUPPRESS PLAN: **LOCK** (suppression-streak-or-high-risk-demands-lock; suppression=SUPPRESS momentum=FREEZE streak=14 driftRisk=HIGH cadence=lane cadence recency:warn offlineOnly=True)
...
- FOCUS: **PORTAL** (portal=79 | alt=1 | pressure=15)
- FOCUS STREAK: **2**
- FOCUS SHIFT: **PRESSURE->PORTAL**
- FOCUS VOL: **STEADY** (switches=2/9 ratio=0.222)
- ROUTE ACTION: **PORTAL_AUDIT** (portal-family tokens dominate top movers)
- ACTION CONF: **LOW** (dom=0.832 spread=64 driftSpread=9)
- FOCUS BAL: **83%** (top=79 total=95 dom=0.832)
- FOCUS ENTROPY: **MID** (norm=0.449 raw=0.711 max=1.585)
...
- DRIFT MOMENTUM: **FLAT** (recent=1.875 older=3.5 delta=-1.625)
- ACTION STABILITY: **WATCH** (retune-watch-needed; conf=LOW vol=STEADY momentum=FLAT)
- PRESSURE LAG: **SLOW** (churn=15 momentum=FLAT |Δ|=1.625)
...
- ROUTE PULSE LINK MODE STREAK: **73** (mode-stable-extended; current=IDLE prior=IDLE priorStreak=72 loaded=True)
...
- ROUTE PULSE LINK MODE FIT: **SYNC** (mode-stable-multi-window; mode=IDLE drift=+0 streak=73)
...
- WHAT-IF: **OFF** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_ALT enabled=False current=PORTAL alt=PRESSURE risk=39->36)
...
- WHAT-IF FIT: **EVEN** (flag-disabled; pressure=HIGH projected=HIGH risk=36 enabled=False)
...
- WHAT-IF FALLBACK FIT: **EVEN** (fallback-flag-disabled; fallback=OFF pressure=HIGH projected=HIGH risk=39 enabled=False)
...
- WHAT-IF FALLBACK ALT2: **NONE** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_FALLBACK_ALT2 enabled=False fallback=NONE scores=portal:79 alt:1 pressure:15)
...
- ARW AUTO PLAN CANDIDATE SUPPRESS: **FALSE** (streak=33 threshold=3)
...
- RGFXWRIU URGENCY FAMILY CHURN: **net +17** (added=19 removed=2 churn=21 coverage=2/2)
...
- DCCFXH: **DCCFXH:AL** (flag=DOTPIO_EXPERIMENT_DMG_COMBO_CONF_FX_ACCENT_TREND_HYS_ALIAS enabled=True rec=ALLOW conf=LOW)
...
- LBA: **OFF** (flag=DOTPIO_EXPERIMENT_LANE_BUCKET_AGE_ALIAS enabled=False sys=999h dw=38h cv=47h)
- LANE BUCKET AGE: **SYSTEMS/OPS 999H | DESIGN/WORLD 38H | COMBAT/VFX 47H** (status=GAP window=24h)
...
- LPR HYS THRESH REC: **LPR HYS THRESH REC:RAISE** (base=12 rec=16 floor=14 ceil=24 priorWindow=True volSpan=1 maxAbsMom=1 ageSpread=961 learn=high volatility widened window using regime step-size memory reason=lane-age volatility is high; raise threshold to suppress flapping)
...
- PULSE HEAT FX COMPACT-BUDGET DRIFT: **STABLE** (no pulse-heat-fx churn in window; compactNet=+76 familyNet=+0 churn=0)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: **STABLE** (no route-glow-fx churn in window; compactNet=+76 familyNet=+0 churn=0)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: **STABLE** (no route-glow rationale rail-mode churn in window; compactNet=+76 familyNet=+0 churn=0)
- STICKY TOKENS: **7**
- ANOMALY: **ON** (sticky=7/3 pressure=15/5)
- ANOMALY CONF: **HIGH** (triggers=2 gap=14)
- ROUTE VIBE DRIFT: **CALM +0 | EDGE +0 | DOOM +0**

## Token Totals (added/removed/net)
- Compact: +81 / -5 / net 76
- Detailed: +55 / -3 / net 52
- Shared: +13 / -3 / net 10
...
- `RGFXWRIU:` net +8 (added 8, removed 0)
...
- RGFXWRIU + ROUTE GLOW FX CONF WHY RAIL INTENSITY WHY CONF URGENCY: +19 / -2 / net 17 (churn=21 coverage=2/2)
...
- DCCFXH ALIAS: DCCFXH:AL (rec=ALLOW conf=LOW flag=DOTPIO_EXPERIMENT_DMG_COMBO_CONF_FX_ACCENT_TREND_HYS_ALIAS enabled=True)
...
- LPR HYS WINDOW: LPR HYS WINDOW:BASE (floor=14, ceil=24, span=10, reason=adaptive window remains near baseline spread)
- LPR HYS WINDOW Δ: +0 (current=BASE, prior=BASE, loaded=True)
- PULSE HEAT FX COMPACT-BUDGET DRIFT: STABLE (compactNet=+76, familyNet=+0, churn=0)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: STABLE (compactNet=+76, familyNet=+0, churn=0)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: STABLE (compactNet=+76, familyNet=+0, churn=0)
