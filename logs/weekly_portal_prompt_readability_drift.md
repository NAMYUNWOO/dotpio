...
- GeneratedAt(UTC): 2026-03-26T04:34:58.942486Z
- Status: **OK**
...
- ARW AUTO PLAN CONF MOMENTUM: **FREEZE** (lock-or-candidate-suppressed; oscillating=False streak=26 confΔ=+0 planΔ=+0 driftRisk=HIGH offlineOnly=True)
- ARW MOMENTUM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_ARW_MOMENTUM_ALIAS enabled=False full=FREEZE)
- ARW MOMENTUM SCORE: **93** (base=85 confΔ=+0 planΔ=+0 streak=26 driftRisk=HIGH parity=LOCK)
...
- AMBIENT RAMP WHY REC CONF STREAK: **26** (suppress=FALSE threshold=3 reason=confidence-streak-extended)
...
- DMG COMBO CONF COACH FALLBACK: **LOCK YOUR BREATH. BANK SAFE HITS.** (guard-or-spike-volatility-regime; streak=2 streakDelta=+1 regime=CALM killHeatVol=0 offlineOnly=True)
...
- PULSE REMAP MOMENTUM SUPPRESS: **SUPPRESS** (freeze-streak-threshold-met; streak=7 threshold=2 suppress=TRUE offlineOnly=True)
- PRMS: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_PULSE_REMAP_MOMENTUM_SUPPRESSION_ALIAS enabled=False full=SUPPRESS)
- PULSE REMAP SUPPRESS PLAN: **LOCK** (suppression-streak-or-high-risk-demands-lock; suppression=SUPPRESS momentum=FREEZE streak=7 driftRisk=HIGH cadence=lane cadence recency:warn offlineOnly=True)
...
- ROUTE PULSE LINK MODE STREAK: **66** (mode-stable-extended; current=IDLE prior=IDLE priorStreak=65 loaded=True)
...
- ROUTE PULSE LINK MODE FIT: **SYNC** (mode-stable-multi-window; mode=IDLE drift=+0 streak=66)
...
- ARW AUTO PLAN CANDIDATE SUPPRESS: **FALSE** (streak=26 threshold=3)
...
- LBA: **OFF** (flag=DOTPIO_EXPERIMENT_LANE_BUCKET_AGE_ALIAS enabled=False sys=999h dw=33h cv=42h)
- LANE BUCKET AGE: **SYSTEMS/OPS 999H | DESIGN/WORLD 33H | COMBAT/VFX 42H** (status=GAP window=24h)
...
- LPR HYS THRESH REC: **LPR HYS THRESH REC:RAISE** (base=12 rec=16 floor=14 ceil=24 priorWindow=True volSpan=2 maxAbsMom=2 ageSpread=966 learn=high volatility widened window using regime step-size memory reason=lane-age volatility is high; raise threshold to suppress flapping)
