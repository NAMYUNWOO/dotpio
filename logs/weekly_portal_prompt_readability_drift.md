...
- GeneratedAt(UTC): 2026-03-26T08:35:57.632457Z
- Status: **OK**
- Window: last 7 days (max 120 commits)
- Checked commits: 120
- Portal prompt commits: 17
- Dominant mode commits: compact=6, detailed=1, neutral=10
- MODE TREND: **COMPACT**
- PRESSURE BAND: **HIGH** (edits +17 / -2 / net 15)
- DRIFT RISK: **HIGH** (score=42 | imbalance=27 | pressure=15)
...
- ARW AUTO PLAN CONF MOMENTUM: **FREEZE** (lock-or-candidate-suppressed; oscillating=False streak=32 confΔ=+0 planΔ=+0 driftRisk=HIGH offlineOnly=True)
- ARW MOMENTUM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_ARW_MOMENTUM_ALIAS enabled=False full=FREEZE)
- ARW MOMENTUM SCORE: **93** (base=85 confΔ=+0 planΔ=+0 streak=32 driftRisk=HIGH parity=LOCK)
...
- AMBIENT RAMP WHY REC CONF STREAK: **32** (suppress=FALSE threshold=3 reason=confidence-streak-extended)
...
- DMG COMBO CONF COACH FALLBACK: **LOCK YOUR BREATH. BANK SAFE HITS. SAME CALL x8.** (streak-rising-lock-in-reminder; streak=8 streakDelta=+1 regime=CALM killHeatVol=0 offlineOnly=True)
...
- PULSE REMAP MOMENTUM SUPPRESS: **SUPPRESS** (freeze-streak-threshold-met; streak=13 threshold=2 suppress=TRUE offlineOnly=True)
- PRMS: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_PULSE_REMAP_MOMENTUM_SUPPRESSION_ALIAS enabled=False full=SUPPRESS)
- PULSE REMAP SUPPRESS PLAN: **LOCK** (suppression-streak-or-high-risk-demands-lock; suppression=SUPPRESS momentum=FREEZE streak=13 driftRisk=HIGH cadence=lane cadence recency:warn offlineOnly=True)
...
- FOCUS: **PORTAL** (portal=87 | alt=1 | pressure=15)
- FOCUS STREAK: **2**
- FOCUS SHIFT: **PRESSURE->PORTAL**
- FOCUS VOL: **STEADY** (switches=2/10 ratio=0.2)
- ROUTE ACTION: **PORTAL_AUDIT** (portal-family tokens dominate top movers)
- ACTION CONF: **LOW** (dom=0.845 spread=72 driftSpread=12)
- FOCUS BAL: **84%** (top=87 total=103 dom=0.845)
- FOCUS ENTROPY: **MID** (norm=0.426 raw=0.675 max=1.585)
...
- DRIFT MOMENTUM: **COOLING** (recent=1.667 older=3.875 delta=-2.208)
- ACTION STABILITY: **WATCH** (retune-watch-needed; conf=LOW vol=STEADY momentum=COOLING)
- PRESSURE LAG: **SLOW** (churn=15 momentum=COOLING |Δ|=2.208)
...
- ROUTE PULSE LINK MODE STREAK: **72** (mode-stable-extended; current=IDLE prior=IDLE priorStreak=71 loaded=True)
...
- ROUTE PULSE LINK MODE FIT: **SYNC** (mode-stable-multi-window; mode=IDLE drift=+0 streak=72)
...
- WHAT-IF: **OFF** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_ALT enabled=False current=PORTAL alt=PRESSURE risk=42->39)
...
- WHAT-IF FIT: **EVEN** (flag-disabled; pressure=HIGH projected=HIGH risk=39 enabled=False)
...
- WHAT-IF FALLBACK FIT: **EVEN** (fallback-flag-disabled; fallback=OFF pressure=HIGH projected=HIGH risk=42 enabled=False)
...
- WHAT-IF FALLBACK ALT2: **NONE** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_FALLBACK_ALT2 enabled=False fallback=NONE scores=portal:87 alt:1 pressure:15)
...
- ARW AUTO PLAN CANDIDATE SUPPRESS: **FALSE** (streak=32 threshold=3)
...
- RGFXWRIU URGENCY FAMILY CHURN: **net +20** (added=22 removed=2 churn=24 coverage=2/2)
...
- LBA: **OFF** (flag=DOTPIO_EXPERIMENT_LANE_BUCKET_AGE_ALIAS enabled=False sys=999h dw=37h cv=46h)
- LANE BUCKET AGE: **SYSTEMS/OPS 999H | DESIGN/WORLD 37H | COMBAT/VFX 46H** (status=GAP window=24h)
...
- LPR HYS THRESH REC: **LPR HYS THRESH REC:RAISE** (base=12 rec=16 floor=14 ceil=24 priorWindow=True volSpan=0 maxAbsMom=0 ageSpread=962 learn=high volatility widened window using regime step-size memory reason=lane-age volatility is high; raise threshold to suppress flapping)
...
- PULSE HEAT FX COMPACT-BUDGET DRIFT: **STABLE** (no pulse-heat-fx churn in window; compactNet=+84 familyNet=+0 churn=0)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: **STABLE** (no route-glow-fx churn in window; compactNet=+84 familyNet=+0 churn=0)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: **STABLE** (no route-glow rationale rail-mode churn in window; compactNet=+84 familyNet=+0 churn=0)
- STICKY TOKENS: **7**
...
- `RGFXWRIU:` net +11 (added 11, removed 0)
...
- RGFXWRIU + ROUTE GLOW FX CONF WHY RAIL INTENSITY WHY CONF URGENCY: +22 / -2 / net 20 (churn=24 coverage=2/2)
...
- LPR HYS WINDOW: LPR HYS WINDOW:BASE (floor=14, ceil=24, span=10, reason=adaptive window remains near baseline spread)
- LPR HYS WINDOW Δ: +0 (current=BASE, prior=BASE, loaded=True)
- PULSE HEAT FX COMPACT-BUDGET DRIFT: STABLE (compactNet=+84, familyNet=+0, churn=0)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: STABLE (compactNet=+84, familyNet=+0, churn=0)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: STABLE (compactNet=+84, familyNet=+0, churn=0)
