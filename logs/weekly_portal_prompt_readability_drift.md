...
- GeneratedAt(UTC): 2026-03-26T02:35:01.682748Z
- Status: **OK**
- Window: last 7 days (max 120 commits)
- Checked commits: 120
- Portal prompt commits: 27
- Dominant mode commits: compact=10, detailed=1, neutral=16
- MODE TREND: **COMPACT**
...
- ARW AUTO PLAN CONF MOMENTUM: **FREEZE** (lock-or-candidate-suppressed; oscillating=False streak=25 confΔ=+0 planΔ=+0 driftRisk=HIGH offlineOnly=True)
- ARW MOMENTUM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_ARW_MOMENTUM_ALIAS enabled=False full=FREEZE)
- ARW MOMENTUM SCORE: **93** (base=85 confΔ=+0 planΔ=+0 streak=25 driftRisk=HIGH parity=LOCK)
...
- AMBIENT RAMP WHY REC CONF STREAK: **25** (suppress=FALSE threshold=3 reason=confidence-streak-extended)
...
- DMG COMBO CONF COACH FALLBACK: **LOCK YOUR BREATH. BANK SAFE HITS.** (guard-or-spike-volatility-regime; streak=1 streakDelta=+1 regime=CALM killHeatVol=0 offlineOnly=True)
...
- PULSE REMAP MOMENTUM SUPPRESS: **SUPPRESS** (freeze-streak-threshold-met; streak=6 threshold=2 suppress=TRUE offlineOnly=True)
- PRMS: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_PULSE_REMAP_MOMENTUM_SUPPRESSION_ALIAS enabled=False full=SUPPRESS)
- PULSE REMAP SUPPRESS PLAN: **LOCK** (suppression-streak-or-high-risk-demands-lock; suppression=SUPPRESS momentum=FREEZE streak=6 driftRisk=HIGH cadence=lane cadence recency:warn offlineOnly=True)
...
- FOCUS: **PORTAL** (portal=160 | alt=1 | pressure=15)
- FOCUS STREAK: **2**
- FOCUS SHIFT: **PRESSURE->PORTAL**
- FOCUS VOL: **STEADY** (switches=2/20 ratio=0.1)
- ROUTE ACTION: **PORTAL_AUDIT** (portal-family tokens dominate top movers)
- ACTION CONF: **LOW** (dom=0.909 spread=145 driftSpread=33)
- FOCUS BAL: **91%** (top=160 total=176 dom=0.909)
- FOCUS ENTROPY: **LOW** (norm=0.297 raw=0.47 max=1.585)
...
- DRIFT MOMENTUM: **FLAT** (recent=2.929 older=2.0 delta=0.929)
- ACTION STABILITY: **WATCH** (retune-watch-needed; conf=LOW vol=STEADY momentum=FLAT)
- PRESSURE LAG: **SLOW** (churn=15 momentum=FLAT |Δ|=0.929)
...
- ROUTE PULSE LINK MODE STREAK: **65** (mode-stable-extended; current=IDLE prior=IDLE priorStreak=64 loaded=True)
...
- ROUTE PULSE LINK MODE FIT: **SYNC** (mode-stable-multi-window; mode=IDLE drift=+0 streak=65)
...
- WHAT-IF: **OFF** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_ALT enabled=False current=PORTAL alt=PRESSURE risk=63->60)
...
- WHAT-IF FIT: **EVEN** (flag-disabled; pressure=HIGH projected=HIGH risk=60 enabled=False)
...
- WHAT-IF FALLBACK FIT: **EVEN** (fallback-flag-disabled; fallback=OFF pressure=HIGH projected=HIGH risk=63 enabled=False)
...
- WHAT-IF FALLBACK ALT2: **NONE** (flag-disabled; flag=DOTPIO_EXPERIMENT_WHAT_IF_FALLBACK_ALT2 enabled=False fallback=NONE scores=portal:160 alt:1 pressure:15)
...
- ARW AUTO PLAN CANDIDATE SUPPRESS: **FALSE** (streak=25 threshold=3)
...
- ROUTE GLOW FX CONF FAMILY CHURN: **net +6** (added=6 removed=0 churn=6 coverage=1/2)
...
- LBA: **OFF** (flag=DOTPIO_EXPERIMENT_LANE_BUCKET_AGE_ALIAS enabled=False sys=999h dw=31h cv=40h)
- LANE BUCKET AGE: **SYSTEMS/OPS 999H | DESIGN/WORLD 31H | COMBAT/VFX 40H** (status=GAP window=24h)
...
- LANE PRIORITY REC CONF: **HIGH** (worstAge=999h momentumGap=8h reason=lane priority signal is strongly separated by age/momentum)
...
- LPR HYS THRESH REC: **LPR HYS THRESH REC:RAISE** (base=12 rec=16 floor=14 ceil=24 priorWindow=True volSpan=8 maxAbsMom=8 ageSpread=968 learn=high volatility widened window using regime step-size memory reason=lane-age volatility is high; raise threshold to suppress flapping)
...
- PULSE HEAT FX COMPACT-BUDGET DRIFT: **STABLE** (no pulse-heat-fx churn in window; compactNet=+157 familyNet=+0 churn=0)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: **STABLE** (no route-glow-fx churn in window; compactNet=+157 familyNet=+0 churn=0)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: **SPIKE** (rail-mode churn is likely competing with compact prompt budget; compactNet=+157 familyNet=+5 churn=9)
- STICKY TOKENS: **9**
...
- Compact: +165 / -8 / net 157
- Detailed: +114 / -5 / net 109
- Shared: +13 / -3 / net 10
...
- ROUTE GLOW FX CONF + RGFXC: +6 / -0 / net 6 (churn=6 coverage=1/2)
...
- LPR HYS WINDOW: LPR HYS WINDOW:BASE (floor=14, ceil=24, span=10, reason=adaptive window remains near baseline spread)
- LPR HYS WINDOW Δ: +0 (current=BASE, prior=BASE, loaded=True)
- PULSE HEAT FX COMPACT-BUDGET DRIFT: STABLE (compactNet=+157, familyNet=+0, churn=0)
- ROUTE GLOW FX COMPACT-BUDGET DRIFT: STABLE (compactNet=+157, familyNet=+0, churn=0)
- ROUTE GLOW FX CONF WHY RAIL MODE COMPACT-BUDGET DRIFT: SPIKE (compactNet=+157, familyNet=+5, churn=9)
