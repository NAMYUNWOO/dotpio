...
- GeneratedAt(UTC): 2026-03-25T02:35:51.172975Z
- Status: **OK**
...
- ARW AUTO PLAN CONF MOMENTUM: **FREEZE** (lock-or-candidate-suppressed; oscillating=False streak=4 confΔ=+0 planΔ=+0 driftRisk=HIGH offlineOnly=True)
- ARW MOMENTUM: **FLAG OFF** (flag=DOTPIO_EXPERIMENT_ARW_MOMENTUM_ALIAS enabled=False full=FREEZE)
- ARW MOMENTUM SCORE: **93** (base=85 confΔ=+0 planΔ=+0 streak=4 driftRisk=HIGH parity=LOCK)
...
- AMBIENT RAMP WHY REC CONF STREAK: **4** (suppress=FALSE threshold=3 reason=confidence-streak-extended)
...
- ROUTE PULSE LINK MODE STREAK: **44** (mode-stable-extended; current=IDLE prior=IDLE priorStreak=43 loaded=True)
...
- ROUTE PULSE LINK MODE FIT: **SYNC** (mode-stable-multi-window; mode=IDLE drift=+0 streak=44)
...
- ARW AUTO PLAN CANDIDATE SUPPRESS: **FALSE** (streak=4 threshold=3)
...
- LBA: **OFF** (flag=DOTPIO_EXPERIMENT_LANE_BUCKET_AGE_ALIAS enabled=False sys=999h dw=7h cv=16h)
