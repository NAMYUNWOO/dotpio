...
- [ ] Systems/QA: Add regression parity assertion linking `CBGCFXWSBPFXPINFBD` payload alias to tooltip row alias.
- [ ] Design/World: Evaluate compact alternate tooltip copy map (`LB lock`, `FB hold`) for localization-safe markdown rollout.
- [ ] Combat/VFX: Explore optional bridge decode FX parity markdown row `CBGCFXWSBPFXPINFBD FX NOTE:<S|E>` under DOS-width constraints.
