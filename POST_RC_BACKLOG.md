...
- [ ] Systems/QA Team: Add payload-only `phaseIntentLegendVersion/hash` metadata signal and regression schema/domain lock for decode-table freshness checks.
- [ ] AI-content/World Team: Prototype optional phase-intent legend microcopy variant line for writer-facing postmortem views (`ANCHOR|SURGE|RECOVER`) with DOS-width guardrails.
