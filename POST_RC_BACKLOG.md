...
- [ ] UX/Design Team: Prototype optional markdown micro-row `CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT` that mirrors payload alias (`L|F`) while preserving <=88 DOS-width and adjacency before `ORDER`.
- [ ] Systems/QA Team: Extend markdown/contract regression to allow optional `THREAT ORDER PATH LEGEND COMPACT` row with 0|2 cardinality and fallback-safe ordering when both threat legend rows are disabled.
