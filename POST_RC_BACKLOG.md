...
- [ ] Systems/Ops + QA Team (injected): add explicit fixture-level domain assertion that `TSDPMFXVWCRITSPMBCBNXDMAPNFXQBACKVFXW` stays within `A|B|C` across summary/token + mixed-window fixtures with first-diverged diagnostics.
- [ ] Design/UX + AI-content Team (injected): add report-only winner legend row for `...NFXQBACKVFXW` (`A=GI/PU/SH|B=GL/PU/SD|C=GN/PS/SD`) and lock adjacency after `...NFXQBACKVFXW`.
