...
- [ ] Combat/VFX + AI-content (injected): Prototype alternate report-only intensity-pack candidate variant (`TSDPMFXVWCRITSPMBCBNXDMAPNFXQ`) for readability fallback.
- [ ] UX/Design (injected): Add DOS-width eval row for intensity-pack decode helper (`TSDPMFXVWCRITSPMBCBNXDMAPNFXPLEN`).
- [ ] Systems/Ops + QA (injected): Add sparse mixed-window fixture assertion that `...MBCBNXDMAPNFXPLEG` mirrors `TSDPMFXVWCRITSPMB` counts.
