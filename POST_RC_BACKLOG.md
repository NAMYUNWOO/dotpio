...
- Injected next backlog:
  - [ ] Design/World + UX Team (injected): add explicit helper-pair parity assertion linking `...LENCUETD` row payload copy to `...LENCUETDLEN` decode-eval payload so wording drift is triaged in one failure path.
