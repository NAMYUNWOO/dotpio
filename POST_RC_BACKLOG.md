...
- [ ] Design/World Team: Optional markdown visibility row + legend for `CBGCFXWSBPFXPINF` after `CBGCFXWSBPFXPIN LEGEND` with strict adjacency contract.
- [ ] Systems/QA Team: Extend markdown/order regression allowance chain to include optional `CBGCFXWSBPFXPINF` row without breaking existing coach-copy adjacency invariants.
- [ ] Systems/Ops Team: Prototype lightweight lane-cadence watchdog artifact that emits underrepresented-lane warning when any lane is 0/10 or untouched >24h.
