...
- [ ] Systems/Ops + QA Team (injected): Add explicit sparse mixed-window fixture tuple entry for `TSDPMFXVWCRITSPMBCBNLEG` so matrix parity mirrors `TSDPMFXVWCRITSPMB` row counts.
- [ ] UX/Design Team (injected): Add compact phase-note decode helper row for `TSDPMFXVWCRITSPMBCBN` (`alias|trend|trendAlias`) and lock DOS-width PASS evaluation.
