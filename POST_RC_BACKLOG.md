...
- [ ] Systems/Ops + QA Team (injected): Add explicit sparse mixed-window fixture parity tuple coverage entry for `TSDPMFXVWCRITSPMBCBH` so matrix tracking mirrors `TSDPMFXVWCRITSPMB` row counts.
- [ ] Combat/VFX + AI-content Team (injected): Prototype offline beat-side alt alias phase-note token (`TSDPMFXVWCRITSPMBCBN`) tied to urgency trend (`UP/FLAT/DOWN`) with runtime coupling disabled.
