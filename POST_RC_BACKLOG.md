...
- [ ] AI Content/Design Team (injected): Prototype alternate concise rationale verb-pack (`ramp/steady/cool`) behind optional flag and compare scanability versus baseline.
...
- [ ] Design/World Team (injected): Prototype optional compact callout legend microcopy variant (`HL=hold lane`, `PE=push edge`, `BC=burst clear`) and evaluate DOS-width/readability against baseline decode row.
- [ ] Systems/Ops Team (injected): Add deterministic cadence override contract that escalates dispatch pressure class when `combat-or-vfx` bucket is missing for two consecutive windows.
