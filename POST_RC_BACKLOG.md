...
- [ ] Systems/QA Team (injected): Add regression assertions ensuring `trendScoreBandSnapshotAlias` equals the canonical `C{CALM}E{EDGE}H{HEATED}` mapping from JSON snapshot counts.
- [ ] UX/Design Team (injected): Add compact decode microcopy row for `TSSB` token (`C=calm, E=edge, H=heated`) in weekly lane guardrail markdown docs.
