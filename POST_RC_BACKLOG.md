...
- [ ] Combat/VFX + Design/World Team (injected): Add compact control-winner cue-priority helper row for `...NFXQBACKSTAF2CTRLWVFX` (`GLINT-HOLD first|PULSE-CUT second|SHIELD-HOLD third`) under <=72 char budget and anchor it before `...CTRLWVFXHLEN`.
- [ ] Systems/Ops + QA Team (injected): Add sparse mixed-window row-count parity assertion that the new `...CTRLWVFX` cue-priority helper row mirrors `...NFXQBACKSTAF2CTRLWVFX` across summary/token sections with first-diverged fixture diagnostics.
