...
- [ ] Combat/VFX: Prototype payload-only `CBGCFXWSBP FX CUE:SOFT|EDGE` adapter so tense storybeat phases request stronger combat feedback pass hints without changing baseline combat tuning.
- [ ] Systems/Ops: Add regression reason-domain lock for storybeat-phase harmonized coach-copy recommendation (`stable-calm-keeps-current-coach-copy|tense-storybeat-prefers-slow-step-before-hold|lock-arc-wobble-needs-anchor-variant|tense-storybeat-lock-arc-promotes-anchor-variant|sway-arc-wobble-needs-stabilizing-variant`).
