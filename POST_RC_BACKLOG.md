...
- [ ] Design/World Team (injected): Add deterministic world/design decode helper pairing row for confidence rollback legend (`...CTRLWNRBLGLEG`) adjacent to `...CTRLWNRBLG` for one-scan readability and 24h design-or-world bucket coverage.
- [ ] Systems/Ops + QA Team (injected): Add sparse mixed-window `...ctrlwnrblglenNonPassRows` diagnostics key and assertion for `...CTRLWNRBLGLEN` payload drift to keep systems-or-ops cadence coverage and first-diverged triage.
