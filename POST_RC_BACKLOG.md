...
## Next Up (Game Director Injection — Cycle IP28)
- [ ] Combat/VFX Team (injected): add fixture-level assertions for `TSDCAD24` domain mapping (`O|W|A` ↔ `OK|WATCH|ALERT`) and markdown row presence parity.
- [ ] Design/World Team (injected): evaluate compact alternate decode copy for `TSDCAD24` legend under <=72 DOS-width and compare scanability vs baseline legend.
- [ ] Systems/Ops + QA Team (injected): add cadence-bucket-specific ops-action dispatch (`combat-or-vfx|design-or-world|systems-or-ops`) while preserving deterministic fallback contract.
