...
- [ ] UX/Design (idea backlog): Add compact alternate helper row `TSDPMFXVWCRITSPMBCBNXDMAPNFXPOA` (`B/E/S`) for one-scan fallback readability A/B.
- [ ] Systems/Ops + QA (idea backlog): Extend sparse mixed-window fixture tuple assertions to include `...NFXPO` and future `...NFXPOA` row-count parity locks.
