...
- [ ] Combat/VFX Team (injected): Add optional compact `NVH` decode suffix legend for INIT-aware callouts (`H/HOLD`, `R/RAMP`, `L/RELIEF`, `S/SHIFT`) with strict DOS-width eval.
- [ ] Design/World Team (injected): Refine cadence helper copy so INIT expansions map to lane verbs in one phrase while preserving existing row ordering.
