...
local function resolveAmbientRamp(routeTag, pressureScore)
    if routeTag == "SAFE" and (tonumber(pressureScore) or 0) <= 2 then
        return "CALM"
    end
    return "TENSE"
end

local function resolveCompactAmbientRamp(ambientRamp)
    if ambientRamp == "CALM" then
        return "C"
    elseif ambientRamp == "TENSE" then
        return "T"
    end
    return nil
end

local function resolveAmbientRampConfidence(routeTag, pressureScore, ambientRamp)
    if ambientRamp == nil then
        return nil
    end
    if ambientRamp == "CALM" then
        return "HIGH"
    end
    if routeTag == "RISK" and (tonumber(pressureScore) or 0) <= 3 then
        return "MID"
    end
    return "LOW"
end

local function resolveCompactAmbientRampConfidence(ambientRampConfidence)
    if ambientRampConfidence == "HIGH" then
        return "H"
    elseif ambientRampConfidence == "MID" then
        return "M"
    elseif ambientRampConfidence == "LOW" then
        return "L"
    end
    return nil
end
...
local function isRouteGlowFxConfidenceWhyCompactPromptExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyCompactAliasExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_COMPACT")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailCompactAliasExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_COMPACT")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailModeExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_MODE")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailIntensityExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_INTENSITY")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailIntensityParityExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_INTENSITY_PARITY")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailIntensityWhyConfParityExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_INTENSITY_WHY_CONF_PARITY")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgencyExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_INTENSITY_WHY_CONF_URGENCY")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgencyFxExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_INTENSITY_WHY_CONF_URGENCY_FX")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgencyCoachExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_ROUTE_GLOW_FX_CONF_WHY_RAIL_INTENSITY_WHY_CONF_URGENCY_COACH")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isPortalAmbientRampExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_PORTAL_AMBIENT_RAMP")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isPortalAmbientRampCompactExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_PORTAL_AMBIENT_RAMP_COMPACT")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isPortalAmbientRampConfidenceExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_PORTAL_AMBIENT_RAMP_CONF")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function isPortalAmbientRampConfidenceCompactExperimentEnabled()
    local raw = os.getenv("DOTPIO_EXPERIMENT_PORTAL_AMBIENT_RAMP_CONF_COMPACT")
    if not raw then
        return false
    end
    local value = string.lower(tostring(raw))
...
local function resolveRouteGlowFxConfidenceWhy(compactRouteGlowFx)
    if compactRouteGlowFx == "SURGE" then
        return "OVERDRIVE"
    elseif compactRouteGlowFx == "SHARP" then
        return "PRESSURE"
...
local function resolveCompactRouteGlowFxConfidenceWhy(routeGlowFxConfidenceWhy)
    if routeGlowFxConfidenceWhy == "OVERDRIVE" then
        return "O"
    elseif routeGlowFxConfidenceWhy == "PRESSURE" then
        return "P"
    elseif routeGlowFxConfidenceWhy == "STABLE" then
        return "S"
    end
    return nil
end

local function resolveRouteGlowFxConfidenceWhyRail(routeGlowFxConfidenceWhy)
    if routeGlowFxConfidenceWhy == "STABLE" then
        return "STEADY"
    elseif routeGlowFxConfidenceWhy == "PRESSURE" or routeGlowFxConfidenceWhy == "OVERDRIVE" then
        return "SPIKE"
    end
    return nil
end
...
local function resolveRouteGlowFxConfidenceWhyRailIntensity(routeGlowFxConfidenceWhyRailMode)
    if routeGlowFxConfidenceWhyRailMode == "LOCK" then
        return "HARD"
    elseif routeGlowFxConfidenceWhyRailMode == "FLEX" then
        return "SOFT"
    end
    return nil
end
...
local function resolveRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgency(routeGlowFxConfidenceWhyRailIntensityWhyConfidence)
    if routeGlowFxConfidenceWhyRailIntensityWhyConfidence == "HIGH" then
        return "HIGH"
    elseif routeGlowFxConfidenceWhyRailIntensityWhyConfidence == "MID" then
        return "MID"
    elseif routeGlowFxConfidenceWhyRailIntensityWhyConfidence == "LOW" then
        return "LOW"
    end
    return nil
end

local function resolveRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgencyFx(routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency)
    if routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency == "HIGH" then
        return "BLAZE"
    elseif routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency == "MID" then
        return "SPARK"
    elseif routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency == "LOW" then
        return "CALM"
    end
    return nil
end

local function resolveRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgencyCoach(routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency)
    if routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency == "LOW" then
        return "STEADY"
    elseif routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency == "MID" or routeGlowFxConfidenceWhyRailIntensityWhyConfUrgency == "HIGH" then
        return "SPIKE"
    end
    return nil
end
                ...
                if compactRouteGlow then
                    ...
                    if compactRouteGlowFx then
                        ...
                        if compactRouteGlowFxConfidence then
                            ...
                            if routeGlowFxConfidenceWhy then
                                local routeGlowFxConfidenceWhyLabel = isRouteGlowFxConfidenceWhyCompactAliasExperimentEnabled() and "RGFXW" or "ROUTE GLOW FX CONF WHY"
                                local compactRouteGlowFxConfidenceWhy = resolveCompactRouteGlowFxConfidenceWhy(routeGlowFxConfidenceWhy)
                                local routeGlowFxConfidenceWhyValue = routeGlowFxConfidenceWhy
                                if routeGlowFxConfidenceWhyLabel == "RGFXW" and compactRouteGlowFxConfidenceWhy then
                                    routeGlowFxConfidenceWhyValue = compactRouteGlowFxConfidenceWhy
                                end
                                appendToken(string.format("%s:%s", routeGlowFxConfidenceWhyLabel, routeGlowFxConfidenceWhyValue), false)
                                if routeGlowFxConfidenceWhyRail then
                                    local routeGlowFxConfidenceWhyRailLabel = isRouteGlowFxConfidenceWhyRailCompactAliasExperimentEnabled() and "RGFXWR" or "ROUTE GLOW FX CONF WHY RAIL"
                                    appendToken(string.format("%s:%s", routeGlowFxConfidenceWhyRailLabel, routeGlowFxConfidenceWhyRail), false)
                                    if routeGlowFxConfidenceWhyRailMode then
                                        appendToken(string.format("RGFXWRM:%s", routeGlowFxConfidenceWhyRailMode), false)
                                        if routeGlowFxConfidenceWhyRailIntensity then
                                            ...
                                            if routeGlowFxConfidenceWhyRailIntensityWhy then
                                                ...
                                                if routeGlowFxConfidenceWhyRailIntensityWhyConfidence then
                                                    ...
                                                    if isRouteGlowFxConfidenceWhyRailIntensityWhyConfidenceUrgencyExperimentEnabled() then
                                                    ...
                                                    if isRouteGlowFxConfidenceWhyRailIntensityWhyConfParityExperimentEnabled() then
                                                        appendToken(string.format("ROUTE GLOW FX CONF WHY RAIL INTENSITY WHY CONF:%s", routeGlowFxConfidenceWhyRailIntensityWhyConfidence), false)
                                                    end
                                                end
                                            end
                                        end
...
function Portal.getTransitionPrompt(maxChars, context)
    ...
    local ambientRamp = nil
    if isPortalAmbientRampExperimentEnabled() then
        ambientRamp = resolveAmbientRamp(routeTag, pressureScore)
    end
    local ambientRampConfidence = nil
    if isPortalAmbientRampConfidenceExperimentEnabled() then
        ambientRampConfidence = resolveAmbientRampConfidence(routeTag, pressureScore, ambientRamp)
    end
    ...
    local prompt = buildTransitionPrompt(routeTag, coach, pressureScore, ambientRamp, ambientRampConfidence, altRouteTag, altDelta, altPlanNudge, altStepCue, altStepConfidence, altStepWhy, altStepWhyConfidence, altStepWhyGlyph, altStepWhyGlyphMode, routeVignette, routeVibeConflict, routeVibeConflictReason, coachOverride, vibeSyncHint, vibeSyncChain, vibeSnapback, vibeRecovery, vibeResilience, vibeDriftWide, vibeDriftGlyph, routePulseMode, vibeTrail, vibeTrailConfidence, vibeTrailConfidenceRail, vibeTrailWhy, vibeTrailArc, vibeTrailWhyConfidence, vibeTrailWhyConfidenceWhy, vibeTrailWhyConfidenceWhyConfidence, vibeTrailWhyConfidenceWhyRail)
    local budget = tonumber(maxChars) or 76
    if budget > 0 and #prompt > budget then
        local compactAmbientRamp = nil
        if isPortalAmbientRampCompactExperimentEnabled() then
            compactAmbientRamp = resolveCompactAmbientRamp(ambientRamp)
        end
        local compactAmbientRampConfidence = nil
        if isPortalAmbientRampConfidenceCompactExperimentEnabled() then
            compactAmbientRampConfidence = resolveCompactAmbientRampConfidence(ambientRampConfidence)
        end
        ...
        local routeGlowFxConfidenceWhy = nil
        if isRouteGlowFxConfidenceWhyCompactPromptExperimentEnabled() then
            routeGlowFxConfidenceWhy = resolveRouteGlowFxConfidenceWhy(compactRouteGlowFx)
        end
        local routeGlowFxConfidenceWhyRail = nil
        if isRouteGlowFxConfidenceWhyRailExperimentEnabled() then
            routeGlowFxConfidenceWhyRail = resolveRouteGlowFxConfidenceWhyRail(routeGlowFxConfidenceWhy)
        end
