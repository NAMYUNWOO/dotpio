...
- [ ] Combat/VFX + AI-content Team (injected): prototype report-only route-tone variant pack (`A=anchor|X=crossfire|S=shelter`) derived from `...NFXQ` for readability A/B (no runtime coupling).
- [ ] Design/World + Systems/QA Team (injected): add DOS-width eval row for `...NFXQBACK` and lock mixed-window parity/order fixture coverage.
