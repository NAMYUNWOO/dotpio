...
- [ ] Systems/Ops + QA Team (injected): Add explicit sparse mixed-window fixture tuple entry for `TSDPMFXVWCRITSPMBCBNLEG` and assert it mirrors `TSDPMFXVWCRITSPMB` row counts in matrix tracking.
- [ ] UX/Design Team (injected): Add compact decode helper row for `TSDPMFXVWCRITSPMBCBN` phase-note format (`alias|trend|trendAlias`) with DOS-width PASS lock.
