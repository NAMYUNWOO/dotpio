...
- [ ] Systems/Ops + QA Team (injected): Add sparse-fixture parity assertion that `TSDCAD24TRICOVSTCMSVHCSTPRLENCUEH` row count mirrors `STPRLENCUEA` in both summary/token sections.
- [ ] Design/World Team (injected): Add one-line helper decode note clarifying `GH/PP` action priority (`hold lane` before `probe lane`) under <=72-char copy budget.
- [ ] AI-content/Combat Team (injected): Prototype offline alternate helper copy variants for `GH->PP` vs `PP->GH` transitions without runtime coupling.
