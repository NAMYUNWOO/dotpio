...
- [ ] Game Director IP31 follow-up (systems/qa): Add explicit domain assertion for `TSDPMFXVWCRITSPMBSAPF` in regression fixture matrix (`PH|HP|ES`) and keep parity with `TSDPMFXVWCRITSPMBSAPN` across mixed-window fixtures.
- [ ] Game Director IP31 follow-up (design/world): Add compact decode helper row documenting adaptive-note transition families (`SURGE/HOLD/COOL` drift groups) with DOS-width evaluation token.
