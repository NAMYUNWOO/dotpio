...
- [ ] Design/World Team (injected): compact copy pass for `...NFXQLEG` (`route` vs `lane`) + `...NFXQLEVAL` baseline/compact token pair under <=72-char budget.
- [ ] Combat/VFX + AI-content Team (injected): prototype offline variant decode micro-pack (`A/X/S`) derived from `AR/XR/SR` without runtime coupling.
