...
- Next injected queue (IP84):
  - [ ] Combat/VFX + AI-content Team (injected): prototype report-only quick-map narrative intensity pack candidate (`...MBCBNXDMAPNFXP`) for A/B readability.
  - [ ] Design/World Team (injected): add decode legend row for compact intensity aliases (`...MBCBNXDMAPNFXALEG`).
  - [ ] Systems/Ops + QA Team (injected): add mixed-window parity/order fixture assertions for `...MBCBNXDMAPNFXA` + `...MBCBNXDMAPNFXALEG`.
