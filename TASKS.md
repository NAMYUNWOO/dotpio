...
- [ ] Systems/Ops + QA Team (injected): add sparse mixed-window first-diverged diagnostics assertion that ALT20 mismatch labels surface fixture + occurrence details in one failure path.
- [ ] Design/World + UX Team (injected): add compact readability comparator row for ALT19 vs ALT20 (`bearing` vs `heading`) with explicit `Bxx|Cxx|LIM72|PREF:*` payload contract.
