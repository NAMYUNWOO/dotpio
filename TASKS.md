...
- [ ] Design/World Team: Add optional digest markdown row + compact legend for `CBGCFXWSBPFXPINF` immediately after `CBGCFXWSBPFXPIN LEGEND` (DOS-width capped).
- [ ] Systems/Ops Team: Add 24h lane-watch payload signal (`vfxTouchedWithin24h`) and regression fixture to auto-highlight stale VFX cadence in director loop outputs.
- [ ] Combat/VFX Team: Add deterministic drift-streak fixture proving `A->S`, `R->E`, `S->H` cue stability across prior-window carryover.
