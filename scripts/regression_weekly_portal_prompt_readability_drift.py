...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="portal-prompt-drift-reg-") as td:
        ...
        assert payload.get("laneCoverageGuardrail") in {"LANE CAP:OK", "LANE CAP:OVER"}, payload
        assert set(payload.get("laneCoverageGuardrailSignals", {}).keys()) == {
            "status",
            "laneCap",
            "capPercent",
            "overCapLanes",
        ...
        assert payload.get("laneCoverageGuardrailSignals", {}).get("laneCap") in {"OK", "OVER"}, payload
        ...
        assert payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentLegendCompactAlias", "").startswith(("FLAG OFF", "CBGCFXWSBPFXPIL:")), payload
        legend_compact_alias_signals = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentLegendCompactAliasSignals", {})
        required_phase_intent_legend_compact_alias_signal_keys = {
            "flagName",
            "flagEnabled",
            "alias",
            "legendVersion",
        ...
        assert required_phase_intent_legend_compact_alias_signal_keys.issubset(set(legend_compact_alias_signals.keys())), payload
        assert legend_compact_alias_signals.get("alias") == "ASR", payload
        assert legend_compact_alias_signals.get("legendVersion") == "v1", payload
        assert legend_compact_alias_signals.get("legendHash") == expected_phase_intent_legend_hash, payload
        assert legend_compact_alias_signals.get("offlineOnly") is True, payload
        assert legend_compact_alias_signals.get("token") == "CBGCFXWSBPFXPIL:ASR", payload
        if legend_compact_alias_signals.get("flagEnabled") is True:
        ...
        assert intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_order_signals.get("adjacencyChain") == "CBGCFXWSBPFXPINF BURST LEGEND->CBGCFXWSBPFXPINF ROUTE LEGEND->CBGCFXWSBPFXPINF BURST DIGEST->CBGCFXWSBPFXPINF ORDER->CBGCFXWSBPFXPI DRILL", payload
        ...
        assert set(payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstSignals", {}).keys()) == {
            ...
            "decodeCopyFallbackLegendAliasPair",
            "decodeCopyFallbackLegendCompactRow",
            "decodeCopyFallbackRouteCompactRow",
            "decodeCopyFallbackRouteLegendCompactRow",
            "decodeCopyFallbackRouteCompactAlias",
            "decodeCopyFallbackRouteCompactAliasToken",
            "decodeCopyFallbackRouteLegendVersion",
            "decodeCopyFallbackRouteLegendHash",
        ...
        assert intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_signals.get("decodeCopyFallbackRouteCompactAlias") == "F1", payload
        assert intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_signals.get("decodeCopyFallbackRouteCompactAliasToken") == "CBGCFXWSBPFXPINFR:F1", payload
        assert intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_signals.get("decodeCopyFallbackRouteLegendVersion") == "v1", payload
        route_legend_hash = intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_signals.get("decodeCopyFallbackRouteLegendHash", "")
        assert isinstance(route_legend_hash, str) and len(route_legend_hash) == 10, f"route legend hash must be 10-char hex: {route_legend_hash}"
        import hashlib as _hashlib_check
        expected_route_hash = _hashlib_check.sha1(b"F1:fallback-v1").hexdigest()[:10]
        assert route_legend_hash == expected_route_hash, f"route legend hash mismatch: {route_legend_hash} != {expected_route_hash}"
        route_legend_row = intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_signals.get("decodeCopyFallbackRouteLegendCompactRow", "")
        assert route_legend_row.startswith("- CBGCFXWSBPFXPINF ROUTE LEGEND:"), f"route legend row format: {route_legend_row}"
        assert "F1=fallback-v1" in route_legend_row, f"route legend row must decode alias: {route_legend_row}"
        assert len(route_legend_row) <= 88, f"route legend row exceeds row budget (88): len={len(route_legend_row)}"
        ...
        assert intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_signals.get("adjacencyChain") == "CBGCFXWSBPFXPINF->CBGCFXWSBPFXPINF BURST->CBGCFXWSBPFXPINF ROUTE LEGEND->CBGCFXWSBPFXPINF ORDER", payload
        ...
        burst_threat_legend_row = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstThreatLegendRow", "")
        burst_threat_legend_row_signals = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstThreatLegendRowSignals", {})
        assert set(burst_threat_legend_row_signals.keys()) == {
            "flagName", "flagEnabled", "legend", "row", "rowFlagOff",
            "dosReadabilityRowBudgetThreshold", "dosRowBudgetWithinThreshold",
            "adjacencyInvariant", "offlineOnly", "runtimeBalanceImpact",
        }, payload
        assert burst_threat_legend_row_signals.get("legend") == "L=LOW,M=MED,H=HIGH", payload
        expected_threat_legend_row = "- CBGCFXWSBPFXPINF THREAT LEGEND: L=LOW,M=MED,H=HIGH"
        expected_threat_legend_row_flag_off = "- CBGCFXWSBPFXPINF THREAT LEGEND: FLAG OFF"
        assert burst_threat_legend_row_signals.get("row") == expected_threat_legend_row, payload
        assert burst_threat_legend_row_signals.get("rowFlagOff") == expected_threat_legend_row_flag_off, payload
        assert burst_threat_legend_row_signals.get("adjacencyInvariant") == "BURST DIGEST->THREAT LEGEND->ORDER", payload
        assert burst_threat_legend_row_signals.get("runtimeBalanceImpact") == "none", payload
        assert burst_threat_legend_row_signals.get("offlineOnly") is True, payload
        assert burst_threat_legend_row_signals.get("dosReadabilityRowBudgetThreshold") == 88, payload
        assert burst_threat_legend_row_signals.get("dosRowBudgetWithinThreshold") is True, payload
        if burst_threat_legend_row_signals.get("flagEnabled") is True:
            assert burst_threat_legend_row == expected_threat_legend_row, payload
        else:
            assert burst_threat_legend_row == expected_threat_legend_row_flag_off, payload
        burst_threat_order_path = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstThreatOrderPath", "")
        burst_threat_order_path_signals = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstThreatOrderPathSignals", {})
        assert set(burst_threat_order_path_signals.keys()) == {
            "flagName", "flagEnabled", "threatLegendEnabled", "path", "token",
            "adjacencyInvariant", "offlineOnly", "runtimeBalanceImpact",
        }, payload
        expected_order_path = "LEGEND" if burst_threat_legend_row_signals.get("flagEnabled") is True else "FALLBACK"
        assert burst_threat_order_path_signals.get("threatLegendEnabled") is (expected_order_path == "LEGEND"), payload
        assert burst_threat_order_path_signals.get("path") == expected_order_path, payload
        assert burst_threat_order_path_signals.get("token") == f"CBGCFXWSBPFXPINF THREAT ORDER PATH:{expected_order_path}", payload
        assert burst_threat_order_path_signals.get("adjacencyInvariant") == "BURST DIGEST->THREAT LEGEND(optional)->ORDER", payload
        assert burst_threat_order_path_signals.get("offlineOnly") is True, payload
        assert burst_threat_order_path_signals.get("runtimeBalanceImpact") == "none", payload
        if burst_threat_order_path_signals.get("flagEnabled") is True:
        ...
        burst_threat_order_path_legend_compact_row = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstThreatOrderPathLegendCompactRow", "")
        burst_threat_order_path_legend_compact_row_signals = payload.get("cadenceBridgeGlyphConfidenceFxPulseMicrocopyWorldToneCoherenceArcStorybeatPhaseFxCueIntensityPulseLanguageVariantPackPhaseIntentNarrationCompactAliasCombatVfxFxCueBurstThreatOrderPathLegendCompactRowSignals", {})
        assert set(burst_threat_order_path_legend_compact_row_signals.keys()) == {
            "flagName", "flagEnabled", "threatOrderPathEnabled", "compactAliasEnabled", "alias", "legend", "row", "rowFlagOff",
            "dosReadabilityRowBudgetThreshold", "dosRowBudgetWithinThreshold", "adjacencyInvariant", "offlineOnly", "runtimeBalanceImpact",
        }, payload
        expected_compact_legend = f"{expected_alias}={'LEGEND' if expected_alias == 'L' else 'FALLBACK'}"
        expected_compact_row = f"- CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT: {expected_compact_legend}"
        expected_compact_row_flag_off = "- CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT: FLAG OFF"
        assert burst_threat_order_path_legend_compact_row_signals.get("alias") == expected_alias, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("legend") == expected_compact_legend, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("row") == expected_compact_row, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("rowFlagOff") == expected_compact_row_flag_off, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("threatOrderPathEnabled") is burst_threat_order_path_signals.get("flagEnabled"), payload
        assert burst_threat_order_path_legend_compact_row_signals.get("compactAliasEnabled") is burst_threat_order_path_legend_compact_signals.get("flagEnabled"), payload
        assert burst_threat_order_path_legend_compact_row_signals.get("adjacencyInvariant") == "THREAT ORDER PATH LEGEND(optional)->THREAT ORDER PATH LEGEND COMPACT->ORDER", payload
        assert burst_threat_order_path_legend_compact_row_signals.get("dosReadabilityRowBudgetThreshold") == 88, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("dosRowBudgetWithinThreshold") is True, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("offlineOnly") is True, payload
        assert burst_threat_order_path_legend_compact_row_signals.get("runtimeBalanceImpact") == "none", payload
        if (
            burst_threat_order_path_legend_compact_row_signals.get("flagEnabled") is True
            and burst_threat_order_path_legend_compact_row_signals.get("compactAliasEnabled") is True
            and burst_threat_order_path_legend_compact_row_signals.get("threatOrderPathEnabled") is True
        ):
        ...
        lane_cap_indices = _find_line_indices("- LANE CAP:")
        ...
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_alias_legend_compact_alias_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPIL:")
        ]
        ...
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPINF BURST LEGEND:")
        ]
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPINF BURST DIGEST:")
        ]
        ...
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT:")
        ]
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND:")
        ]
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPINFBD TOOLTIP:")
        ]
        cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_indices = [
            i for i, line in enumerate(md_lines) if line.startswith("- CBGCFXWSBPFXPINFBD FX NOTE:")
        ]
        ...
        assert len(lane_cap_indices) == 2, (
            "expected exactly two LANE CAP rows (summary + token-coverage sections)"
        )
        ...
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_alias_legend_compact_alias_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPIL rows (summary + token-coverage sections)"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_alias_legend_compact_alias_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_alias_legend_indices), (
            "expected CBGCFXWSBPFXPIL rollout rows to appear only when CBGCFXWSBPFXPI LEGEND rows are present"
        )
        ...
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINF BURST LEGEND rows (summary + token-coverage sections)"
        )
        ...
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINF BURST DIGEST rows (summary + token-coverage sections)"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_indices), (
            "expected CBGCFXWSBPFXPINF BURST DIGEST rollout rows to appear only when CBGCFXWSBPFXPINF BURST LEGEND rows are present"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINF THREAT LEGEND rows (summary + token-coverage sections)"
        )
        ...
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT rows (summary + token-coverage sections)"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices), (
            "expected CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT rollout rows to appear only when CBGCFXWSBPFXPINF BURST DIGEST rows are present"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND rows (summary + token-coverage sections)"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices), (
            "expected CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND rollout rows to appear only when CBGCFXWSBPFXPINF BURST DIGEST rows are present"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINFBD TOOLTIP rows (summary + token-coverage sections)"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices), (
            "expected CBGCFXWSBPFXPINFBD TOOLTIP rollout rows to appear only when CBGCFXWSBPFXPINF BURST DIGEST rows are present"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_indices) in {0, 2}, (
            "expected zero or exactly two CBGCFXWSBPFXPINFBD FX NOTE rows (summary + token-coverage sections)"
        )
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices), (
            "expected CBGCFXWSBPFXPINFBD FX NOTE rollout rows to appear only when CBGCFXWSBPFXPINFBD TOOLTIP rows are present"
        )
        ...
        for burst_digest_idx in cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices:
            ...
            assert len(burst_digest_row) <= burst_digest_row_budget, (
                "expected CBGCFXWSBPFXPINF BURST DIGEST row to stay within DOS-width budget "
                f"(<= {burst_digest_row_budget}, got {len(burst_digest_row)}): {burst_digest_row}"
            )
        ...
        threat_legend_row_budget = 88
        for threat_legend_idx in cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_indices:
            threat_legend_row = md_lines[threat_legend_idx]
        ...
        threat_order_path_legend_compact_row_budget = 88
        for threat_order_path_legend_compact_idx in cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_indices:
            threat_order_path_legend_compact_row = md_lines[threat_order_path_legend_compact_idx]
            assert len(threat_order_path_legend_compact_row) <= threat_order_path_legend_compact_row_budget, (
                "expected CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT row to stay within DOS-width budget "
                f"(<= {threat_order_path_legend_compact_row_budget}, got {len(threat_order_path_legend_compact_row)}): {threat_order_path_legend_compact_row}"
            )
        threat_order_bridge_legend_row_budget = 88
        for threat_order_bridge_legend_idx in cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_indices:
            threat_order_bridge_legend_row = md_lines[threat_order_bridge_legend_idx]
            assert len(threat_order_bridge_legend_row) <= threat_order_bridge_legend_row_budget, (
                "expected CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND row to stay within DOS-width budget "
                f"(<= {threat_order_bridge_legend_row_budget}, got {len(threat_order_bridge_legend_row)}): {threat_order_bridge_legend_row}"
            )
        threat_order_bridge_decode_tooltip_row_budget = 88
        for threat_order_bridge_decode_tooltip_idx in cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices:
            threat_order_bridge_decode_tooltip_row = md_lines[threat_order_bridge_decode_tooltip_idx]
            assert len(threat_order_bridge_decode_tooltip_row) <= threat_order_bridge_decode_tooltip_row_budget, (
        ...
        threat_order_bridge_decode_fx_note_row_budget = 88
        for threat_order_bridge_decode_fx_note_idx in cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_indices:
            threat_order_bridge_decode_fx_note_row = md_lines[threat_order_bridge_decode_fx_note_idx]
            assert len(threat_order_bridge_decode_fx_note_row) <= threat_order_bridge_decode_fx_note_row_budget, (
                "expected CBGCFXWSBPFXPINFBD FX NOTE row to stay within DOS-width budget "
                f"(<= {threat_order_bridge_decode_fx_note_row_budget}, got {len(threat_order_bridge_decode_fx_note_row)}): {threat_order_bridge_decode_fx_note_row}"
            )
            if "FLAG OFF" not in threat_order_bridge_decode_fx_note_row:
                expected_parity_alias = "S" if burst_threat_order_bridge_decode_copy_signals.get("bridgeAlias") == "L" else "E"
                assert threat_order_bridge_decode_fx_note_row == f"- CBGCFXWSBPFXPINFBD FX NOTE:{expected_parity_alias}", (
                    "expected CBGCFXWSBPFXPINFBD FX NOTE row to preserve bridge decode parity mapping (L->S, F->E)"
                )
        ...
        assert len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_order_indices) <= len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices), (
            "expected CBGCFXWSBPFXPINF ORDER rollout rows to appear only when CBGCFXWSBPFXPINF BURST DIGEST rows are present"
        )
        ...
        for section_idx, (section_name, drift_idx, momentum_idx, momentum_family_churn_idx, storybeat_idx, storybeat_family_churn_idx, storybeat_phase_idx, storybeat_phase_family_churn_idx, storybeat_phase_fx_cue_idx, storybeat_phase_fx_cue_compact_alias_idx, storybeat_phase_fx_cue_compact_alias_intensity_idx, storybeat_phase_fx_cue_compact_alias_intensity_pulse_alias_idx, storybeat_phase_fx_cue_intensity_pulse_decode_microline_pair_idx, storybeat_phase_fx_cue_compact_alias_intensity_pulse_alias_legend_idx, storybeat_phase_fx_cue_compact_alias_intensity_legend_idx, storybeat_phase_fx_cue_compact_alias_intensity_coach_copy_idx, coach_copy_variant_rec_idx, coach_copy_variant_rec_compact_alias_idx, coach_copy_variant_rec_compact_alias_legend_idx, coach_copy_reason_priority_alias_idx, coach_copy_reason_priority_alias_legend_idx, coherence_alias_idx) in enumerate(zip(
            ...
            expected_spacer_prefixes = [
                ...
                "- CBGCFXWSBPFXPINF BURST LEGEND:",
                "- CBGCFXWSBPFXPINF ROUTE:",
                ...
                "- CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT:",
                "- CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND:",
                "- CBGCFXWSBPFXPINFBD TOOLTIP:",
                "- CBGCFXWSBPFXPINFBD FX NOTE:",
            ...
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_prefix_idx = optional_spacer_prefix_to_index["- CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT:"]
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_prefix_idx = optional_spacer_prefix_to_index["- CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND:"]
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_prefix_idx = optional_spacer_prefix_to_index["- CBGCFXWSBPFXPINFBD TOOLTIP:"]
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_prefix_idx = optional_spacer_prefix_to_index["- CBGCFXWSBPFXPINFBD FX NOTE:"]
            ...
            optional_phase_intent_alias_legend_compact_alias_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_alias_legend_compact_alias_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_alias_legend_compact_alias_indices) == 2
                else None
            )
            ...
            if optional_phase_intent_alias_legend_compact_alias_idx is not None:
                assert optional_phase_intent_alias_legend_idx is not None, (
                    f"markdown contract violated in {section_name} section: CBGCFXWSBPFXPIL row cannot appear without CBGCFXWSBPFXPI LEGEND row"
                )
                assert optional_phase_intent_alias_legend_compact_alias_idx == optional_phase_intent_alias_legend_idx + 1, (
                    f"markdown contract violated in {section_name} section: expected CBGCFXWSBPFXPIL row directly after CBGCFXWSBPFXPI LEGEND row"
                )
            if optional_phase_intent_alias_legend_copy_idx is not None:
                expected_prior_idx = optional_phase_intent_alias_legend_compact_alias_idx if optional_phase_intent_alias_legend_compact_alias_idx is not None else optional_phase_intent_alias_legend_idx
                expected_prior_label = "CBGCFXWSBPFXPIL" if optional_phase_intent_alias_legend_compact_alias_idx is not None else "CBGCFXWSBPFXPI LEGEND"
                ...
                assert optional_phase_intent_alias_legend_copy_idx == expected_prior_idx + 1, (
                    f"markdown contract violated in {section_name} section: expected CBGCFXWSBPFXPI LEGEND COPY row directly after {expected_prior_label} row"
                )
            ...
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_indices) == 2
                else None
            )
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_indices) == 2
                else None
            )
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_indices[section_idx]
            ...
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_indices) == 2
                else None
            )
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_indices) == 2
                else None
            )
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_indices) == 2
                else None
            )
            optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_idx = (
                cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_indices[section_idx]
                if len(cadence_bridge_glyph_conf_fx_pulse_microcopy_world_tone_coherence_arc_storybeat_phase_fx_cue_intensity_pulse_language_variant_pack_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_indices) == 2
                else None
            )
            ...
            if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx is not None:
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_idx is not None, (
                    f"markdown contract violated in {section_name} section: CBGCFXWSBPFXPINF BURST LEGEND row cannot appear without CBGCFXWSBPFXPINF BURST row"
                )
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx == optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_idx + 1, (
                    f"markdown contract violated in {section_name} section: expected CBGCFXWSBPFXPINF BURST LEGEND row directly after CBGCFXWSBPFXPINF BURST row"
                )
            if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx is not None:
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx is not None, (
                    f"markdown contract violated in {section_name} section: CBGCFXWSBPFXPINF BURST DIGEST row cannot appear without CBGCFXWSBPFXPINF BURST LEGEND row"
                )
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx >= optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx + 1 and optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx <= optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx + 3, (
                    f"markdown contract violated in {section_name} section: expected CBGCFXWSBPFXPINF BURST DIGEST row after CBGCFXWSBPFXPINF BURST LEGEND row (with optional ROUTE + ROUTE LEGEND spacers)"
                )
            if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_idx is not None:
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx is not None, (
                    f"markdown contract violated in {section_name} section: CBGCFXWSBPFXPINF THREAT LEGEND row cannot appear without CBGCFXWSBPFXPINF BURST DIGEST row"
                )
            ...
            if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_idx is not None:
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx is not None, (
                    f"markdown contract violated in {section_name} section: CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT row cannot appear without CBGCFXWSBPFXPINF BURST DIGEST row"
                )
                expected_compact_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_idx or optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx
                expected_compact_prior_label = (
                    "CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND"
                    if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_idx is not None
                    else "CBGCFXWSBPFXPINF BURST DIGEST"
                )
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_idx == expected_compact_prior_idx + 1, (
                    f"markdown contract violated in {section_name} section: expected CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT row directly after {expected_compact_prior_label} row"
                )
            ...
            if optional_rehearsal_hint_idx is not None:
                ...
                elif optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx is not None:
                    expected_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_legend_idx
                    expected_prior_label = "CBGCFXWSBPFXPINF BURST LEGEND"
            ...
            if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_order_idx is not None:
                assert optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx is not None, (
                    f"markdown contract violated in {section_name} section: CBGCFXWSBPFXPINF ORDER row cannot appear without CBGCFXWSBPFXPINF BURST DIGEST row"
                )
                expected_order_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_idx or optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_digest_idx
                expected_order_prior_label = (
                    "CBGCFXWSBPFXPINF THREAT LEGEND"
                    if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_legend_idx is not None
                    else "CBGCFXWSBPFXPINF BURST DIGEST"
                )
                ...
                if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_idx is not None:
                    expected_order_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_path_legend_compact_idx
                    expected_order_prior_label = "CBGCFXWSBPFXPINF THREAT ORDER PATH LEGEND COMPACT"
                if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_idx is not None:
                    expected_order_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_legend_idx
                    expected_order_prior_label = "CBGCFXWSBPFXPINF THREAT ORDER BRIDGE LEGEND"
                if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_idx is not None:
                    expected_order_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_tooltip_idx
                    expected_order_prior_label = "CBGCFXWSBPFXPINFBD TOOLTIP"
                if optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_idx is not None:
                    expected_order_prior_idx = optional_phase_intent_narration_compact_alias_combat_vfx_fx_cue_burst_threat_order_bridge_decode_fx_note_idx
                    expected_order_prior_label = "CBGCFXWSBPFXPINFBD FX NOTE"
        ...
        for section_idx, (miss_risk_idx, alias_idx, cadence_24h_idx, lane_cap_idx, watchdog_idx, watchdog_streak_idx, coach_idx, coach_why_idx, coach_why_alias_idx, coach_why_hyst_alias_idx, coach_why_hyst_rec_idx, coach_why_hyst_rec_conf_idx, coach_why_hyst_rec_conf_alias_idx, coach_why_hyst_rec_conf_floor_idx, coach_why_hyst_rec_conf_floor_alias_idx, coach_why_hyst_rec_conf_floor_fx_pulse_idx, coach_why_hyst_rec_conf_floor_fx_pulse_legend_idx, cadence_bridge_idx, coach_why_hyst_rec_conf_floor_family_churn_idx, coach_why_hyst_rec_conf_floor_fx_pulse_family_churn_idx, coach_why_hyst_rec_conf_floor_fx_pulse_family_trend_idx, cadence_bridge_family_churn_idx, coach_why_hyst_family_churn_idx, coach_why_hyst_rec_family_churn_idx, coach_why_hyst_rec_conf_family_churn_idx, coach_why_family_churn_idx, coach_alias_idx, coach_family_churn_idx, watchdog_legend_idx) in enumerate(
            ...
            assert lane_cap_idx == cadence_24h_idx + 1, (
                f"expected LANE CAP row directly after LANE CADENCE 24H CHECK row in section {section_idx}"
            )
            assert watchdog_idx == lane_cap_idx + 1, (
                f"expected COMBAT/VFX CADENCE WATCHDOG row directly after LANE CAP row in section {section_idx}"
            )
