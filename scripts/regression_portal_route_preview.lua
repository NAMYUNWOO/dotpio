...
expect(prompt:find("FX:SURGE"), "transition prompt should include high-pressure portal FX cue for combat readability")
expect(prompt:find("ALT ROUTE:RISK"), "transition prompt should include adaptive lower-pressure route suggestion when pressure is high")
expect(prompt:find("ALT DELTA:-1", 1, true), "transition prompt should include adaptive pressure delta token for suggested alternate route")
...
expect(unknownPrompt:find("FX:CALM"), "low-pressure route prompt should expose calm portal FX cue")
expect(not unknownPrompt:find("ALT ROUTE:"), "unknown low-pressure route should not include adaptive alt-route hint")
