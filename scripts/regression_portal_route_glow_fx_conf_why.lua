-- Regression: compact route-glow FX confidence rationale token experiment.
...
expect(lowPrompt:find("ROUTE GLOW FX CONF WHY:STABLE", 1, true), "low-pressure compact prompt should include STABLE rationale")
...
expect(midPrompt:find("ROUTE GLOW FX CONF WHY:PRESSURE", 1, true), "mid-pressure compact prompt should include PRESSURE rationale")
Portal.cancelTransition()
...
expect(highPrompt:find("ROUTE GLOW FX CONF WHY:OVERDRIVE", 1, true), "high-pressure compact prompt should include OVERDRIVE rationale")
Portal.cancelTransition()

print("[PASS] portal compact route glow FX confidence rationale regression validated")
