...
expect(stable:find("RGFXWRI WHY CONF:LOW", 1, true), "SAFE path should emit LOW confidence")
expect(stable:find("RGFXWRIU:LOW", 1, true), "SAFE path should emit LOW urgency")

local pressure = promptFor("RISK", 160, "ASH")
expect(pressure:find("RGFXWRI WHY CONF:MID", 1, true), "RISK path should emit MID confidence")
expect(pressure:find("RGFXWRIU:MID", 1, true), "RISK path should emit MID urgency")

local overdrive = promptFor("SPIKE", 175, "ASH")
expect(overdrive:find("RGFXWRI WHY CONF:HIGH", 1, true), "SPIKE path should emit HIGH confidence")
expect(overdrive:find("RGFXWRIU:HIGH", 1, true), "SPIKE path should emit HIGH urgency")

Portal._setRouteTagOverride("02", nil)
print("[PASS] portal route glow FX confidence rationale rail intensity why confidence urgency regression validated")
